      ...
      <div className="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
        ...
        <header className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <div className={cn("led", connected && "on")} />
            <div>
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <button
              ...
              className={cn(
                ...
                outputOn
                  ? "bg-primary text-primary-foreground border-primary shadow-[0_0_30px_-5px_var(--primary)]"
        ...
        <section className="panel p-5 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <EditableValue label="OVP" value={state.ovp} unit="V" decimals={2} step={0.1} max={35}
              disabled={!connected} onCommit={(v) => dev?.setOvp(v)} />
            ...
            <EditableValue label="OTP" value={state.otp} unit="°C" decimals={0} step={1} max={120}
              disabled={!connected} accent="current" onCommit={(v) => dev?.setOtp(v)} />
            <EditableValue label="LVP" value={state.lvp} unit="V" decimals={2} step={0.1} max={30}
              disabled={!connected} onCommit={(v) => dev?.setLvp(v)} />
          </div>
        </section>
